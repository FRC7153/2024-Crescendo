...
public class RevBlinkin {
     ...
     */
    public void setSignal(double signal) { pwm.set(signal); }

    /**
     * Set the color of the LED strip
     * @param color
     */
    public void setConstantColor(BlinkinSolidColor color) { pwm.set(color.signal); }
    ...
    public void setColorPattern(BlinkinPattern pattern, int color) {
        pwm.set(-0.03 + (0.2 * pattern.index) + ((color == 1) ? 0.0 : 0.2));
    }
     ...
     * @param pattern
     */
    public void setMulticolorPattern(BlinkinMulticolorPattern pattern) { pwm.set(pattern.signal); }
